...
if(isset($_GET['filter']) && $_GET['filter']=='kecamatan') {

  $sql = "SELECT * FROM company WHERE kecamatan='$_GET[search]'";

  $result = $conn->query($sql);
  if($result->num_rows > 0) {
    while($row1 = $result->fetch_assoc()) {
      $sql1 = "SELECT * FROM buat_lowongan WHERE id_company>='$row1[id_company]' LIMIT $start_from, $limit";
  ...
  if(isset($_GET['filter']) && $_GET['filter']=='searchBar') {
    ...
    $search = $_GET['search'];
    $sql = "SELECT * FROM buat_lowongan WHERE jobtitle LIKE '%$search%' LIMIT $start_from, $limit";
    

  } else if(isset($_GET['filter']) && $_GET['filter']=='experience') {

    $sql = "SELECT * FROM buat_lowongan WHERE experience>='$_GET[search]' LIMIT $start_from, $limit";
      ...
      $sql1 = "SELECT * FROM company WHERE id_company='$row[id_company]'";
         ...
         <div class="attachment-block clearfix">
                ...
                <div class="attachment-pushed">
                  ...
                  <div class="attachment-text">
                      <div><strong><?php echo $row1['companyname']; ?> | <?php echo $row1['kecamatan']; ?> | Pengalaman kerja <?php echo $row['experience']; ?> Years</strong></div>
                  </div>
                </div>
              </div>
